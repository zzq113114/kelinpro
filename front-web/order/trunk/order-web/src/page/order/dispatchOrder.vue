  ...
  <div class="sys-page">
    <div class="page-content">
      ...
      <el-row>
        <el-col :span="24">
          <div class="grid-content bg-purple-dark">
            <el-button type="primary" @click="showAddModelView">派单增加</el-button>
          </div>
        </el-col>
      </el-row>
      <app-section>
        <el-table :data="tableData" style="width: 100%" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="customerName"  label="下单人"></el-table-column>
           <el-table-column prop="createDate" show-overflow-tooltip  label="下单日期"></el-table-column>
          ...
          <el-table-column prop="staffsName" show-overflow-tooltip label="派单员工"></el-table-column>
            <el-table-column  prop="amount" show-overflow-tooltip label="支付金额"></el-table-column>
          <el-table-column  label="操作">
            <template slot-scope="scope">
              <el-button v-if="scope.row.status!=2&&scope.row.status!=5" size="small" type="primary" @click="againDisOrder(scope.row)">重派</el-button>
              <!-- <el-button v-if="scope.row.status==1" size="small" type="primary" @click="showDispatchOrder(scope.row)">分派</el-button> -->
              <el-button v-if="scope.row.status==2" size="small" type="primary" @click="showFinishOrder(scope.row)">完成</el-button>
              <el-button v-if="scope.row.status!=2&&scope.row.status!=6&&scope.row.status!=3&&scope.row.status!=4" size="small" type="primary" @click="showFinishOrder(scope.row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    <el-dialog title="新增派单" :visible.sync="dialog.addModel.show" :modal-append-to-body="false" custom-class="editPawDialog">
      <el-form :model="addModel" :rules="orderRules" ref="addModel" label-width="200px">
        <el-form-item label="接单员" prop="orderTaker">
          <el-input v-model="addModel.orderTaker" auto-complete="off" style="width:30%"></el-input>
        </el-form-item>
        ...
        <el-form-item label="工作时长" prop="workHours">
          <el-input v-model.number="addModel.workHours" auto-complete="off" style="width:30%"></el-input>
        </el-form-item>
    ...
    <el-dialog title="派单修改" :visible.sync="dialog.editModel.show" :modal-append-to-body="false" custom-class="editPawDialog">
      <el-form :model="addModel" :rules="orderRules" ref="addModel" label-width="200px">
        <el-form-item label="下单日期" prop="createDate">
           <el-date-picker  v-model="addModel.createDate"  type="datetime"   placeholder="选择日期时间"></el-date-picker>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    againDisOrder(row) {
      this.$confirm('确认重新下派该订单吗？').then(_=>{
        console.log(_);
        let _self = this;
        this.$ajax({
          method:'GET',
          url: this.GLOBAL.REQUEST_HOST + "order/againOrder/"+row.id
        }).then(function(res){
    ...
    selectOrderList() {
      ...
      this.$ajax({
        method: "POST",
        url: this.GLOBAL.REQUEST_HOST + "order/orderPage",
